---
title: "Проанализировать клиентов в выборке, провести кластеризацию, сделать описание сегментов"
...
## Самые поплярные автомобили для кластера 1 (Кластер который самый убыточный для страховщика). И регион
...
dat$raw_f %>% filter(cluster == cl) %>% group_by(INSURER_GENDER) %>% 
  count(POLICY_CLM_GLT_N) %>% summarise(median = median(VEHICLE_ENGINE_POWER))
...
* 1 Кластер, наиболее аварийные случаи, с более премиальными авто, практически всегда являются причиной ДТП, аварийность этой группы не зависит от пола. В основном место регистрации ТС Москва, самый низкий медианный возраст среди кластеров, 39 лет для мужчины и 40 лет для женщины.

* Как видно из графика страхователь из кластера 1 убыточен для компании, ниже попробуем прогнозировать убытки от полиса для страхователя из кластера 1
